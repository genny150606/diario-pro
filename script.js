...
const StorageManager = {
    ...
    const TaskManager = {
        ...
        toggleTask(id) {
            ...
            if (task) {
                ...
                // GAMIFICATION HOOK
                if (task.completed) {
                    GamificationManager.addXP(10, 'Compito Completato! ✅');
                }
            }
        },
    ...
    const GradeManager = {
        ...
        addGrade(grade) {
            ...
            GamificationManager.addXP(xp, `Voto Aggiunto: ${grade.value}`);
    ...
    const PomodoroManager = {
        ...
        completePhase() {
            if (this.mode === 'study') {
                ...
                // GAMIFICATION HOOK
                GamificationManager.addXP(25, 'Sessione Studio Completata 🍅');
    ...
    const GamificationManager = {
        levelThresholds: [0, 100, 300, 600, 1000, 1500, 2500, 5000, 10000],
        levelNames: ['Principiante', 'Studioso', 'Impegnato', 'Dedito', 'Maestro', 'Saggio', 'Erudito', 'Genio', 'Leggenda'],
        ...
        getXP() {
            const appData = StorageManager.load();
            return appData.stats.xp || 0;
        },

        getNextLevelXP() {
            const currentLevel = this.getCurrentLevel();
            return this.levelThresholds[currentLevel] || 100000; // Fallback for max level
        },

        addXP(amount, reason = 'Attività completata') {
            const appData = StorageManager.load();
            if (!appData.stats.xp) appData.stats.xp = 0;

            appData.stats.xp += amount;
            StorageManager.save(appData);
        ...
        checkLevelUp() {
            const appData = StorageManager.load();
            const currentXP = appData.stats.xp;
            const currentLevel = appData.stats.level || 1;

            let newLevel = currentLevel;
            // Check if XP exceeds threshold for NEXT level
            // Level 1 -> Threshold[1] is for Level 2
            for (let i = currentLevel; i < this.levelThresholds.length; i++) {
                if (currentXP >= this.levelThresholds[i]) {
                    newLevel = i + 1;
                } else {
                    break;
                }
            ...
            if (newLevel > currentLevel) {
                ...
                this.updateUI(); // Refresh UI immediately
            }
        },
        ...
        showLevelUpModal(level) {
            ...
            const titleDisplay = document.querySelector('.level-up-title');

            if (overlay && levelDisplay) {
                levelDisplay.textContent = level;
                // Add name if available
                const name = this.levelNames[level - 1] || 'Studente';
                if (titleDisplay) titleDisplay.innerHTML = `LEVEL UP! <br><span style="font-size:0.6em; color:white;">${name}</span>`;
        ...
        showToast(amount, message) {
            const toast = document.getElementById('xpToast');
            const amountEl = document.getElementById('xpToastAmount');
            const msgEl = document.getElementById('xpToastMessage');

            if (toast && amountEl && msgEl) {
        ...
        renderGamificationPage() {
            ...
            const prevLevelXP = this.levelThresholds[currentLevel - 1] || 0;
            const progress = Math.min(100, Math.max(0, ((currentXP - prevLevelXP) / (nextLevelXP - prevLevelXP)) * 100));

            // HERO SECTION
            const heroLevel = document.getElementById('gamePageLevel');
            const heroTitle = document.getElementById('gamePageTitle');
            const heroFill = document.getElementById('gamePageXpFill');
            const heroXp = document.getElementById('gamePageXp');
            const heroNextXp = document.getElementById('gamePageNextXp');

            if (heroLevel) heroLevel.textContent = currentLevel;
            if (heroTitle) heroTitle.textContent = this.levelNames[currentLevel - 1] || 'Studente';
            if (heroFill) heroFill.style.width = `${progress}%`;
            if (heroXp) heroXp.textContent = currentXP;
            if (heroNextXp) heroNextXp.textContent = nextLevelXP;

            // STATS GRID
            const totalXpEl = document.getElementById('totalXpValue');
            const streakEl = document.getElementById('streakValue');
            const notesXpValue = document.getElementById('notesXpValue');

            if (totalXpEl) totalXpEl.textContent = currentXP;
            // Mockup streak for now
            if (streakEl) streakEl.textContent = '3 🔥';

            // Count notes/flashcards for display (simplified logic)
            const appData = StorageManager.load();
            if (notesXpValue) notesXpValue.textContent = (appData.notes || []).length;
            const flashXpValue = document.getElementById('flashcardsXpValue');
            if (flashXpValue) flashXpValue.textContent = (appData.flashcards || []).length;

            // ACHIEVEMENTS LOGIC (Mockup)
            const achFirstNote = document.getElementById('achFirstNote');
            if (achFirstNote && (appData.notes || []).length > 0) achFirstNote.classList.add('unlocked');

            const achFirstFlash = document.getElementById('achFirstFlashcard');
            if (achFirstFlash && (appData.flashcards || []).length >= 5) achFirstFlash.classList.add('unlocked');

            const achLevel5 = document.getElementById('achLevel5');
            if (achLevel5 && currentLevel >= 5) achLevel5.classList.add('unlocked');
        },
        ...
        triggerConfetti() {
            ...
            for (let i = 0; i < 50; i++) {
                ...
                if (overlay) overlay.appendChild(confetti);
    ...
    const AppManager = {
        ...
        setupEventListeners() {
            ...
            // Navigation
            document.querySelectorAll('.sidebar-item, .nav-link, .nav-item').forEach(btn => {
...
/* ============================================
   ...
   ============================================ */
// Ensure these functions are globally available for inline HTML calls
window.updateDashboard = () => {
    if (typeof AppManager !== 'undefined') {
        AppManager.renderDashboard();
        GamificationManager.updateUI();
    }
};
...
// INITIALIZATION
document.addEventListener('DOMContentLoaded', () => {
    // Only init if not already initialized by another script
